<script>
  ...
  function simulationUpdate() {
    ...
    // if the link contains spiderman add it to first links
    links.forEach((d) => {
      if (d.source.id === spiderID) {
          firstLinks.push(d.target.id);
      }
      if (d.target.id === spiderID) {
          firstLinks.push(d.source.id);
      }
    });

    // if the node is in a first link add it to first nodes
    nodes.forEach((d, i) => {
        if (firstLinks.includes(d.id)) {
          firstNodes.push(d.id)
          if (d.id !== spiderID) {
            d.spidernumber = 1;
          }
        }
        ...
        if (d.id === spiderID) {
          spiderNode = d;
          d.spidernumber = 0;
          //console.log(activeNode.spidernumber);
        }
    });

    // if the link contains a first node and non first node add it to second links
    links.forEach((d) => {
      // if the source is a firstnode and target isnt in firstnodes
      if (firstNodes.includes(d.source.id) && !firstNodes.includes(d.target.id)) {
          secondLinks.push(d.target.id);
          secondNodes.push(d.target.id);
      }
      // if the target is a firstnode and the source isnt in firstnodes
      if (firstNodes.includes(d.target.id) && !firstLinks.includes(d.source.id)) {
          secondLinks.push(d.source.id);
          secondNodes.push(d.source.id);
      }
    });
    links.forEach((d) => {
    // if the source is a firstnode and target isnt in firstnodes
      if (secondNodes.includes(d.source.id) && !secondNodes.includes(d.target.id)) {
          thirdLinks.push(d.target.id);
          thirdNodes.push(d.target.id);
      }
      // if the target is a firstnode and the source isnt in firstnodes
      if (secondNodes.includes(d.target.id) && !secondLinks.includes(d.source.id)) {
          thirdLinks.push(d.source.id);
          thirdNodes.push(d.source.id);
      }
    });
    nodes.forEach((d, i) => {
        if (!secondNodes.includes(d.id) && !firstNodes.includes(d.id) || d.id === spiderID) {
          context.beginPath();
          context.moveTo(d.x, d.y);
          context.lineTo(spiderNode.x, spiderNode.y);
        ...
        else if (!secondNodes.includes(d.id) && !firstNodes.includes(d.id)) {
          console.log(d)
        }
...
<div on:resize={resize} class="container">
  {#if activeNode}
    <breadcrumb id="nodeDetails">
      ...
      <br>
      <!--Spider Number: {activeNode.spidernumber}-->
